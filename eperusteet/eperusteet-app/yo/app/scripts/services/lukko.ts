...
angular
    ...
    .factory("LukkoRakenne", function(SERVICE_LOC, $resource) {
        return $resource(
            SERVICE_LOC + "/perusteet/:osanId/suoritustavat/:suoritustapa/rakenne/lukko/",
            {
                osanId: "@osanId",
            ...
            },
            {
                multiple: {
                    method: "GET",
                    url: SERVICE_LOC + "/perusteet/:osanId/suoritustavat/:suoritustapa/lukko/:tyyppi"
                }
            }
        );
    })
    ...
    .factory("LukkoLukioAihekokonaisuus", function(SERVICE_LOC, $resource) {
        return $resource(
            SERVICE_LOC + "/perusteet/:perusteId/lukiokoulutus/aihekokonaisuudet/:aihekokonaisuusId/lukko",
            {
    ...
    .factory("LukkoOppiaineenVuosiluokkakokonaisuus", function(SERVICE_LOC, $resource) {
        return $resource(
            SERVICE_LOC +
                "/perusteet/:perusteId/perusopetus/oppiaineet/:oppiaineId/vuosiluokkakokonaisuudet/:vuosiluokkaId/lukko",
            {
    ...
    .service("Lukitus", function(
        ...
        LukkoLukioOppiaine,
        LukkoLukiokurssi,
        LukkoLukioAihekokonaisuudet,
        PerusopetusService,
        LukiokoulutusService,
        LukkoOppiaineenVuosiluokkakokonaisuus,
        LukkoPerusteenosaByTutkinnonOsaViite,
        LukkoVuosiluokkakokonaisuus,
        LukkoLaajaalainenOsaaminen,
        LukkoLukioRakenne,
        LukkoLukioAihekokonaisuus,
        LukkoLukioYleisetTavoitteet,
        LukkoAIPELaajaalainenOsaaminen
    ) {
        var lukitsin = null;
        var etag = null;
        var vapautin = null;
        ...
        var genericResourcesByState = {
            "root.perusteprojekti.suoritustapa.lukioosaalue": function() {
                switch ($stateParams.osanTyyppi) {
                    case "oppiaineet_oppimaarat":
                        return LukkoLukioOppiaine;
                    case "aihekokonaisuudet":
                        return LukkoLukioAihekokonaisuudet;
                    default:
                        return null;
                }
            },
        ...
        function tilaToLukkoParams(): any {
            ...
            if (
                $state.current.name === "root.perusteprojekti.suoritustapa.osaalue" &&
                $stateParams.osanTyyppi &&
                $stateParams.osanId !== "uusi"
            ) {
        ...
        function genericLukitse(cb) {
            ...
            if (lukkotyyppi) {
                ...
                if (params) {
                    ...
                    // Lukkoa ei tarvita koska kyseessä uusi
                    var d = $q.defer();
                    d.resolve();
                    if (cb) {
                        cb();
        ...
        function lukitseLukioKurssi(id, cb, editointiCheck) {
            return lukitse(
                LukkoLukiokurssi,
                { perusteId: parseInt(LukiokoulutusService.getPerusteId(), 10), kurssiId: id },
                cb,
                editointiCheck
            );
        }
        ...
        function lukitseLukiorakenne(cb, editointiCheck) {
            return lukitse(
                LukkoLukioRakenne,
                { perusteId: parseInt(LukiokoulutusService.getPerusteId(), 10) },
                cb,
                editointiCheck
            );
        }
        ...
        function lukitseLukioAihekokonaisuus(id, cb) {
            return lukitse(
                LukkoLukioAihekokonaisuus,
                { perusteId: LukiokoulutusService.getPerusteId(), aihekokonaisuusId: id },
                cb
            );
        }
