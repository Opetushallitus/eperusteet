...
angular
    .module("eperusteApp")
    .controller("arviointiCtrl", function ($scope,
                                           $translate,
                                           YleinenData,
                                           Varmistusdialogi,
                                           $timeout,
        ...
        $scope.arviointiasteikkoChanged = function (kohdealue) {
            ArviointiPreferences.setting(
                "asteikko",
                kohdealue.$newkohde.arviointiasteikko
            );
        };
        ...
        $scope.kohdealue = {
            uusi: function () {
                ...
                if (
                    angular.isUndefined($scope.arviointi) ||
        ...
        $scope.kohde = {
            muokkaa: function (kohde, event) {
                ...
                if (_.isArray(kriteerit) && _.isEmpty(kriteerit)) {
                    valmisteleKriteerit(
                        kriteerit,
                        $scope.arviointiasteikot[kohde._arviointiAsteikko].osaamistasot
                    );
                }
            },
            ...
            cahngeArviointiasteikko: arvioinninkohde => {
                ...
                if (parseInt($scope.editableKohde._arviointiAsteikko) === parseInt(arvioinninkohde.$vanhaArviointiAsteikko)) {
                    return;
                }
            ...
            uusi: function (kohdealue) {
                if (
                    angular.isUndefined(kohdealue.arvioinninKohteet) ||
                    kohdealue.arvioinninKohteet === null
                ) {
                ...
                valmisteleKriteerit(
                    ...
                    kohdealue.$newkohde.arviointiasteikko.osaamistasot
                );
            ...
            poistuMuokkauksesta: function (list, index) {
                ...
                _.each($scope.editableKohde.osaamistasonKriteerit, function (kriteeri) {
                    if (
                        kriteeri.kriteerit.length === 1 &&
                        !Utils.hasLocalizedText(kriteeri.kriteerit[0])
                    ) {
                        kriteeri.kriteerit = [];
                    }
                });
        ...
        if (
            $scope.eiKohdealueita &&
            (angular.isUndefined($scope.arviointi) || $scope.arviointi === null)
        ) {
    ...
    .directive("arviointi", function (YleinenData, $timeout, TutkinnonOsaLeikelautaService) {
        return {
            ...
            link: function (scope: any) {
                scope.eiKohdealueita =
                    scope.eiKohdealueita === "true" || scope.eiKohdealueita === true;
                scope.editAllowed =
                    scope.editAllowed === "true" || scope.editAllowed === true;
