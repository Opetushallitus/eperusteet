...
angular
            ...
            function() {
                _.each($scope.datat.opetus.lapset, function(area) {
                    ...
                    Algoritmit.kaikilleLapsisolmuille(area, "lapset", function(lapsi) {
                        lapsi.$url = $state.href("root.perusteprojekti.suoritustapa.lukioosaalue", {
                ...
                Algoritmit.kaikilleLapsisolmuille($scope.peruste.sisalto, "lapset", function(lapsi) {
                    switch (lapsi.perusteenOsa.osanTyyppi) {
                        ...
                        case "opetuksenyleisettavoitteet":
                            lapsi.$url = $state.href("root.perusteprojekti.suoritustapa.lukioosat", {
                                osanTyyppi: "opetuksen_yleiset_tavoitteet"
                            });
                            break;
                        case "aihekokonaisuudet":
                            lapsi.$url = $state.href("root.perusteprojekti.suoritustapa.lukioosat", {
                                osanTyyppi: "aihekokonaisuudet"
                            });
                            break;
    ...
    .controller("LukioOsalistausController", function(
        $scope,
        $state,
        $stateParams,
        LukiokoulutusService,
    ...
    .controller("LukioOsaAlueController", function(
        $scope,
        $q,
        $stateParams,
        LukiokoulutusService,
        ...
        if ($stateParams.osanId !== "uusi") {
            Kommentit.haeKommentit(KommentitByPerusteenOsa, {
                ...
                perusteenOsaId: $stateParams.osanId
            });
        }
    ...
    .controller("LukioOppiaineKurssiPuuController", function(
        $scope,
        $stateParams,
        $q,
        $translate,
        ...
        const collapseFun = defaultCollapse => n => {
        ...
        const isHidden = node => !node || node.$$hide || (node.$$nodeParent && node.$$nodeParent.$$collapsed);
        ...
        var acceptMove = function(node, to) {
            ...
            return (
                ((node.dtype === "oppiaine" && to.root && !node.$$nodeParent) ||
                    (node.dtype === "oppiaine" &&
                        to.dtype === "oppiaine" &&
                        to.koosteinen &&
                        !node.koosteinen &&
        ...
        var initPages = function(arr, pagination: PaginationDetails): void {
            ...
            if (!pagination.changePage) {
                pagination.changePage = function(pageNum: number) {
                    changePage(arr, pagination, pageNum);
                };
            }
        ...
        Editointikontrollit.registerCallback({
            ...
            save: function(kommentti) {
                LukioKurssiService.updateOppiaineKurssiStructure(
                    $scope.treeRoot,
                    $scope.liittamattomatKurssit,
                    kommentti
                ).then($state.reload);
            ...
            resolve({
                ...
                template: n => {
                    var handle = treehandleTemplate(),
                        collapse =
                            !$scope.treehelpers.editMode || n.dtype === "oppiaine"
                        ...
                        icon = "";
                    if (n.dtype === "kurssi") {
                        var remove = $scope.treehelpers.editMode
        ...
        $scope.liittamattomatOsatProvider = $q(function(resolve) {
            var templateAround = function(tmpl) {
                return (
                    '<div class="liittamaton-kurssi recursivetree tree-list-item bubble-osa empty-item"' +
                    '          ng-show="!node.$$hide && node.$$pagingShow">' +
                    tmpl +
                    "</div>"
                );
            };
            resolve({
                ...
                template: function(n) {
                    return templateAround(
                        '<div class="puu-node kurssi-node">' +
                            treehandleTemplate() +
                            kurssiColorbox() +
                            timestampTemplate() +
