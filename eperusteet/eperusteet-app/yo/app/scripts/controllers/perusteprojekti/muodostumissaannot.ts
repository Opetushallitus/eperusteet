...
.controller('PerusteprojektiMuodostumissaannotCtrl', ($scope, $stateParams, $timeout, PerusteenRakenne, Notifikaatiot,
    Editointikontrollit, PerusteProjektiService, Kommentit, KommentitBySuoritustapa, Lukitus, VersionHelper,
    Muodostumissaannot, virheService, PerusteProjektiSivunavi, perusteprojektiTiedot, $q, Varmistusdialogi, Algoritmit,
    PerusteenOsat) => {
                ...
                if (versio && !$scope.versiot.latest) {
                    if (!revNumber) {
                        ...
                        PerusteenRakenne.haeTutkinnonosatVersioByPeruste($scope.peruste.id, $scope.suoritustapa, revNumber, function (tutkinnonOsat) {
                            ...
                            VersionHelper.changeRakenne($scope.versiot, {
                                id: $scope.peruste.id,
                                suoritustapa: $scope.suoritustapa
                            }, function (response) {
                    ...
                    const res = await PerusteenRakenne.haeByPerusteprojekti($stateParams.perusteProjektiId, $scope.suoritustapa);
                    successCb(res);
                    resolve();
                }
            })
    ...
    function haeVersiot(force = false, cb = _.noop) {
        VersionHelper.getRakenneVersions($scope.versiot, {
            id: $scope.peruste.id,
            suoritustapa: $scope.suoritustapa
        }, force, cb);
    }
